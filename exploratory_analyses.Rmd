...
This rise in aid worker attacks is disproportionately affecting national staff. The next two plots show rising national staff victims and the proportion of total victims who are national staff, while the proportion of international staff victims has decreased. 
...
Finally, this plot shows victims of the top 5 most dangerous countries over time. Context for the spike in attacks in Afghanistan in 2013 are discussed in the accompanying case study. A rise in attacks in Syria corresponds to the start of the Syrian Civil War in 2011.  
...
As we see below, most aidworker attacks occur during transit, on roadways. The large portion of 'unknown' attack locations and attack types is indicative of challenges in collecting comprehensive data during humanitarian emergencies.   
...
aidworker_df %>%
  ...
  geom_bar(stat="identity") + 
  coord_flip() + 
  labs(title = "Attack locations and types of attacks",
...
plot_context = 
  ...
  geom_bar(stat="identity") + 
  labs(x = "Country",
       y = "Number of Attacks (Context)") + 
  theme(axis.text.x = element_text(angle = -90, vjust = 0.5, hjust=1)) +
  theme(legend.text=element_text(size=10)) + 
  theme(legend.title = element_blank())

plot_type + plot_context + 
plot_annotation(title = "Most Dangerous Countries: Attack Types (Left) and Contexts (Right)")
