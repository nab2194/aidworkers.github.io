...
This map shows the number of incidents in each country, from the Aid Worker Security Database, since 1997. Hover over each country to see more details about attacks in each country for a given year. Drag the bar to see the number of incidents mapped out for different years.

*Note that data from Mauritius is excluded due to the available extent of the map. Our dataset includes one incident in Mauritius in 2017, an assault with one national victim.
...
renderLeaflet({
  ...
  labels = glue::glue("<strong>{map_data()$name}</strong><br/>{map_data()$number_incidents} total incidents<br/>{map_data()$tot_victims} victims<br/>{map_data()$tot_national} national victims<br/>{map_data()$tot_intl} international victims") %>%
  purrr::map(htmltools::HTML)
